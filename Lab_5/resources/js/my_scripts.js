...
*/
		function loadStatsPage(){
			...
			var l = 0;
			for(i = 2; i < rows; i++){
				if(t.rows[i].cells[2].innerHTML > t.rows[i].cells[3].innerHTML){
					t.rows[i].cells[4].innerHTML = "CU"
					wins++;
				}
				else{
					t.rows[i].cells[4].innerHTML = t.rows[i].cells[1].innerHTML;
					l++;
				}
			}
			var d = document.getElementById('wins');
...
*/
	function loadPlayersPage(){
		...
		btn.innerHTML = players[i].name;
		document.getElementById('player_selector').appendChild(btn);
	...
	function switchPlayers(playerNum){
		...
		document.getElementById('p_yards').innerHTML = players[playerNum].yards;
